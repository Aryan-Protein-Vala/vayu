...
async def _http_probe(url: str, timeout: float = 3.0) -> bool:
...
async def _probe_cached(key: str, url: str) -> bool:
    async with _get_lock():
        now = time.time()
        if _cache[key] is None or now - _cache["ts"] > CACHE_TTL:
            _cache[key] = await _http_probe(url)
            _cache["ts"] = time.time()
...
async def sarvam_available() -> bool:
    return await _probe_cached("sarvam", "https://api.sarvam.ai")
